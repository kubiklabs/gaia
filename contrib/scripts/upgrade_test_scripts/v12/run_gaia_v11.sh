...
cp ./build/junod11 "$NODE_HOME"/cosmovisor/genesis/bin/junod
$BINARY init upgrader --chain-id $CHAINID --home "$NODE_HOME"


if ! test -f "./build/junod12"; then
  echo "junod v12 does not exist"
  exit
fi

mkdir -p "$NODE_HOME"/cosmovisor/upgrades/v12/bin
cp ./build/junod12 "$NODE_HOME"/cosmovisor/upgrades/v12/bin/junod
