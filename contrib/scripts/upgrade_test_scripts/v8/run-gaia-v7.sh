...
if ! test -f "./build/junod7"; then
  echo "junod v7 does not exist"
  exit
fi
...
cp ./build/junod7 "$NODE_HOME"/cosmovisor/genesis/bin/junod
$BINARY init upgrader --chain-id $CHAINID --home "$NODE_HOME"


if ! test -f "./build/junod8"; then
  echo "junod v8 does not exist"
  exit
fi

mkdir -p "$NODE_HOME"/cosmovisor/upgrades/v8-Rho/bin
cp ./build/junod8 "$NODE_HOME"/cosmovisor/upgrades/v8-Rho/bin/junod
